  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          // 7bit - По каким-то причинам в некоторых случаях эта ошибка компиляции имеет позицию 0:0.
          // Эта проверка - обходной путь - если позиция 0, то остаются значения
          // позиции по умолчанию, берущиеся из парсера. Но это тоже не совсем
          // корректно, т. к. столбец не может определится правильно - курсор
          // находится далеко в конце строки.
          if Params[c].Val.Pos > 0 then
          begin
          //
...
{$ENDIF}
  ...
  begin
    // 7bit //
    if MType = nil then raise EPSCompilerException.Create(RPS_UnknownError +
      ' at ' + IntToStr(FParser.Row) + ':' + IntToStr(FParser.Col));
    //
    ...
    begin
      ...
      or (ImplicitPeriod) do begin
        ...
        begin
          ...
          ImplicitPeriod := False;
          FType := GetTypeNo(BlockInfo, p);
          if FType is TPSVariantType then  // 7bit
...
//        MakeError('', ecTypeMismatch, '');
    ...
    begin
      // 7bit
      {Если в скрипте встречался такой код:
      const c = '123'+
        '456'
        '789';
      то возникало AV.}
      if (p as TConstData).Data = nil then Exit(nil);
      //
