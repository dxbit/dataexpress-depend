  ...
  try
    case desttype.BaseType of
      ...
      btU8:
        begin
          // 7bit
          // Значение, отличное от 0 и 1 доставляет проблемы в LCL и проводит к
          // сбою. Например: TMenuItem.Visible := V (-1) приводит к AV
          if desttype.ExportName = 'BOOLEAN' then
            tbtu8(Dest^) := Ord(PSGetUInt(Src, srctype) <> 0)
          else
          //
          ...
          case var1Type.BaseType of
            ...
            btProcPtr:
              begin
                ...
                begin
                  if {Longint}IPointer(Var1^) = 0 then       //7bit
            ...
            btS32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btS16: b := tbts32(var1^) <> tbts16(Var2^);
                  //7bit
                  btProcPtr: b := tbts32(var1^) <> LongInt(IPointer(Var2^));
                  btU32: b := tbts32(var1^) <> Longint(tbtu32(Var2^));
      ...
      {$ENDIF}
        ...
        case Dest.aType.BaseType of
	      ...
	      {$endif}
              ...
              tbtu32(Pointer(IPointer(dest.p)+PointerSize)^) := 0; *)
              {$ifdef FPC_REQUIRES_PROPER_ALIGNMENT}
              IPointer(dest.p^) := unaligned(IPointer((@FData^[FCurrentPosition])^));
	      {$else}
              IPointer(dest.p^) := IPointer((@FData^[FCurrentPosition])^);
	      {$endif}
              IPointer(Pointer(IPointer(dest.p)+PointerSize)^) := 0;
              IPointer(Pointer(IPointer(dest.p)+PointerSize)^) := 0;
              //
              Inc(FCurrentPosition, 4);
            end;
            ...
            begin
              ...
              begin
                case PPSVariantPointer(tmp).DestType.BaseType of
                  ...
                  // 7bit
                  btu32: Param := tbtu32(PPSVariantPointer(tmp).DataDest^);
                  btProcPtr: Param := IPointer(PPSVariantPointer(tmp).DataDest^);
...
begin
  ...
  case Longint(p.Ext1) of
    ...
    1: Stack.SetInt(-1, SysUtils.StrToInt(Stack.GetAnsiString(-2))); // strtoint
    2: Stack.SetInt(-1, SysUtils.StrToIntDef(Stack.GetAnsiString(-2), Stack.GetInt(-3))); // strtointdef
    3:
{$IFNDEF PS_NOWIDESTRING}
...
{$ENDIF}      
    ...
    23: Stack.SetReal(-1, SysUtils.StrToFloat(Stack.GetAnsiString(-2))); // StrToFloat
    24: Stack.SetAnsiString(-1, SysUtils.FloatToStr(Stack.GetReal(-2)));// FloatToStr
    25:
{$IFNDEF PS_NOWIDESTRING}
    if Stack.GetItem(Stack.Count -2)^.FType.BaseType = btUnicodeString then
...
begin
  ...
  // using register calling convention        (???? 7bit ????)
  {$IFDEF CPU64}
  PPSVariantU32(IntVal).Data := 1;// Int64(FSelf); 7bit  (Это параметр CreateNewInstance: Boolean) Он будет в RSI (Linux64)
  {$ELSE}
  PPSVariantU32(IntVal).Data := Cardinal(FSelf);
  FSelf := pointer(1);    // 7bit
  {$ENDIF}
  // FSelf := pointer(1); 7bit
  {$ELSE}
...
begin
  try
    ...
    begin
      ...
      begin
        SetMethodProp(TObject(FSelf), PPropInfo(p.Ext1), MkMethod(Caller, {tbtu32}IPointer(n.dta^)));   // 7bit
      end else
      case n.aType.BaseType of
        ...
        btClass: SetOrdProp(TObject(FSelf), P.Ext1, {Longint}IPointer(n.Dta^));            // 7bit
	  {$IFDEF DELPHI6UP}
{$IFNDEF PS_NOWIDESTRING}
...
{$ENDIF}
      ...
      begin
        ...
        {Cardinal}IPointer(n.Dta^) := GetMethodNo(m, Caller);              // 7bit
        if {Cardinal}IPointer(n.dta^) = 0 then                             // 7bit
        begin
      ...
      case n.aType.BaseType of
        ...
        btClass: {Longint}IPointer(n.dta^) := GetOrdProp(TObject(FSelf), p.Ext1);   // 7bit
	  {$IFDEF DELPHI6UP}
{$IFNDEF PS_NOWIDESTRING}
        {$IFDEF DELPHI2009UP}
...
//{$DEFINE empty_methods_handler}        // 7bit
{$ENDIF}

{$ifdef fpc}
  {$if defined(cpupowerpc) or defined(cpuarm))}       // 7bit
    {$define empty_methods_handler}
  {$elseif defined(cpu64)}                            // 7bit
    {$define fpc_64_methods_handler}                  // 7bit
  {$ifend}
{$endif}
...
type
  __PARAM_RECORD = packed record
    _xmm7_a, _xmm7_b: Pointer;
    _xmm6_a, _xmm6_b: Pointer;
    _xmm5_a, _xmm5_b: Pointer;
    _xmm4_a, _xmm4_b: Pointer;
    _xmm3_a, _xmm3_b: Pointer;
    _xmm2_a, _xmm2_b: Pointer;
    _xmm1_a, _xmm1_b: Pointer;
    _xmm0_a, _xmm0_b: Pointer;
    R9, R8, RCX, RDX, RSI, RDI, RAX: Pointer;
    Rest: array [0..0] of Pointer;
  end;
  PPARAM_RECORD = ^__PARAM_RECORD;
...
begin
  ...
  begin
    ...
    cpt := Self.Se.GetTypeNo(StrToInt(e));
    //--Pointers etc.
    if ((fmod = '%') or (fmod = '!') or (AlwaysAsVariable(cpt))) and (RegNo < 6) then
    begin
      tmp := CreateHeapVariant(self.Se.FindType2(btPointer));
      PPSVariantPointer(tmp).DestType := cpt;
      Params[i] := tmp;
      case regno of
        ...
        1: begin
            PPSVariantPointer(tmp).DataDest := paramRecord^.RSI;
            inc(regno);
          end;
        2: begin
            PPSVariantPointer(tmp).DataDest := paramRecord^.RDX;
      ...
      case regno of
        0: begin
            if (cpt.BaseType=btSingle) or (cpt.BaseType=btDouble) then
              ...
              CopyArrayContents(@PPSVariantData(tmp)^.Data, @paramRecord^.RDI, 1, cpt);
            inc(regno);
          end;
        1: begin
            if (cpt.BaseType=btSingle) or (cpt.BaseType=btDouble) then
              ...
              CopyArrayContents(@PPSVariantData(tmp)^.Data, @paramRecord^.RSI, 1, cpt);
            inc(regno);
          end;
        2: begin
            if (cpt.BaseType=btSingle) or (cpt.BaseType=btDouble) then
        ...
        4: begin
            if (cpt.BaseType=btSingle) or (cpt.BaseType=btDouble) then
              CopyArrayContents(@PPSVariantData(tmp)^.Data, @paramRecord^._xmm4_a, 1, cpt)
            else
              CopyArrayContents(@PPSVariantData(tmp)^.Data, @paramRecord^.R8, 1, cpt) ;
            inc(regno);
          end;
        5: begin
            if (cpt.BaseType=btSingle) or (cpt.BaseType=btDouble) then
              CopyArrayContents(@PPSVariantData(tmp)^.Data, @paramRecord^._xmm5_a, 1, cpt)
            else
              CopyArrayContents(@PPSVariantData(tmp)^.Data, @paramRecord^.R9, 1, cpt) ;
            inc(regno);
          end;
...
asm
  ...
  //sub RSP, 64    //Create Savehouse for storing registers...
...
begin
  ...
  begin
    // 7bit
    if TPSTypeRec(p1^).BaseType = btVariant then
    begin
      if FVarCounter > 0 then
      begin
...
begin
  ...
  Result.FType := aType;
  InitializeVariant(Pointer(IPointer(Result)+PointerSize), aType);
  if aType.BaseType = btVariant then Inc(FVarCounter);
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          // 7bit - AnsiString выдает кракозябры. Заменил на Utf8String
          DispParam.rgvarg[i].bstrVal := StringToOleStr(Utf8String(Par[High(Par)-i]));
...
begin
  FRealSize := 2 * sizeof(Pointer) + Sizeof(IPointer{Cardinal}); // 7bit
end;
