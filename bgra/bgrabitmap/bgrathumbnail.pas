...
// 7bit. Доработка, чтобы миниатюра не была больше самого изображения. При
// размере миниатюры в 64 пикселя, оригинал размером в 32 пикселя должен так и
// оставаться - 32 пикселя.
...
begin
  ...
  try
    ...
    begin
      if (ABitmap.Width < AWidth) and (ABitmap.Height < AHeight) then
      begin
        AWidth := ABitmap.Width;
        AHeight := ABitmap.Height;
      end;
      factorX := AWidth/(ABitmap.Width*AHorizShrink);
      factorY := AHeight/(ABitmap.Height*AVerticalShrink);
