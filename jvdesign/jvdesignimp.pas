...
type
  ...
  public
    ...
    procedure UTF8KeyPress(var UTF8Key: TUTF8Char); override; // 7bit
    //{$IFDEF COMPILER9_UP}
    //procedure PaintMenu;
    //{$ENDIF COMPILER9_UP}
...
// 7bit: не срабатывает - потомок не вызывает.
...
begin
  ...
  DsgnControls := TList.Create;
  for i := 0 to Cnt.ControlCount - 1 do
  begin
    C := Cnt.Controls[i];
    if not (C is TJvDesignHandle) then
      DsgnControls.Add(C);
  end;

  for i := 0 to DsgnControls.Count - 1 do
  begin
    //C := Cnt.Controls[i];
    //if C is TJvDesignHandle then Continue;
    C := TControl(DsgnControls[i]);
    P := Point(C.Left, C.Top);
    if C.Parent <> Surface.Container then
      P := C.Parent.ClientToParent(P, Surface.Container);
    ...
    begin
      if Surface.Selector.IsSelected(C) then
        Surface.Selector.RemoveFromSelection(C)
      else
...
begin
  ...
  begin
    ...
    Dec(X, X mod Surface.StepX);
    Y := FMouseLast.Y - FMouseStart.Y;
    Dec(Y, Y mod Surface.StepY);
  end;
end;
...
begin
  ...
  // 7bit Выравниваем по сетке
  if (Surface.Count = 1) and ((Delta.X <> 0) or (Delta.Y <> 0)) then
  begin
    FDragRects[0] := Surface.Selection[0].BoundsRect;
    X := FDragRects[0].Left;
    Y := FDragRects[0].Top;
    X := X - (X div Surface.StepX * Surface.StepX);
    Y := Y - (Y div Surface.StepY * Surface.StepY);
    Dec(Delta.X, X);
    Dec(Delta.Y, Y);
    OffsetRect(FDragRects[0], Delta.X, Delta.Y);
  end
  else
    for I := 0 to Surface.Count - 1 do
...
begin
  ...
  begin
    Surface.DoChangingComponents; // 7bit

    CalcDragRects;
    for I := 0 to Surface.Count - 1 do
    begin
      if Surface.Selection[I] <> Surface.Container then
...
begin
  ...
  case FHandleId of
    dhLeftTop, dhMiddleTop, dhRightTop:
      Y := FDragRect.Top - (FDragRect.Top div Surface.StepY * Surface.StepY);
    dhLeftBottom, dhMiddleBottom, dhRightBottom:
      Y := FDragRect.Bottom - (FDragRect.Bottom div Surface.StepY * Surface.StepY);
  end;
  case FHandleId of
    dhLeftTop, dhLeftMiddle, dhLeftBottom:
      X := FDragRect.Left - (FDragRect.Left div Surface.StepX * Surface.StepX);
    dhRightTop, dhRightMiddle, dhRightBottom:
      X := FDragRect.Right - (FDragRect.Right div Surface.StepX * Surface.StepX);
  end;
  Delta := GetMouseDelta;
  Dec(Delta.X, X);
  Dec(Delta.Y, Y);
